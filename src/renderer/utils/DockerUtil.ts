...
export default {
		...
		this.client.listImages((err, list) => {
			if (err) {
				...
				list.map((image, idx) => {
					...
					list[idx].inUse = _.indexOf(this.imagesUsed, imgSha) !== -1;
					let imageSplit = "";
					if (image.RepoTags) {
	...
	updateContainer(name, data) {
		...
		existing.inspect((error, existingData) => {
			...
			const fullData = _.extend(existingData, data);
	...
	rename(name, newName) {
		this.client.getContainer(name).rename({name: newName}, (error) => {
			...
			const oldPath = util.windowsToLinuxPath(path.join(util.home(), util.documents(), "Kitematic", name));
			const newPath = util.windowsToLinuxPath(path.join(util.home(), util.documents(), "Kitematic", newName));
	...
	startLinkedContainers(name, callback) {
		...
		this.client.getContainer(name).inspect((error, container) => {
			if (error) {
				...
				const links = _.map(container.HostConfig.Links, (link, key) => {
					return link.split(":")[0].split("/")[1];
				});

				async.map(links, function(link, cb) {
					const linkedContainer = self.client.getContainer(link);
					if (linkedContainer) {
		...
		container.unpause( () => {
			container.kill( () => {
				container.remove( (error) => {
					...
					containerServerActions.destroyed({id: name});
					const volumePath = path.join(util.home(), "Kitematic", name);
	...
	listen() {
		...
		this.client.getEvents((error, stream) => {
			...
			stream.on("data", (json) => {
				...
				if ((["pull", "untag", "tag", "delete", "attach"] as any).includes(data.status)) {
					this.refresh();
				}
